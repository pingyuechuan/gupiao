...
// scripts/orderbook-test.ts
init_define_import_meta_env();

// src/services/batchQuote.ts
init_define_import_meta_env();
...
// src/services/http.ts
var http3 = axios_default.create({
  timeout: 15e3,
  headers: {
...
init_define_import_meta_env();
var DEFAULT_PROVIDER = define_import_meta_env_default.VITE_DEFAULT_PROVIDER || "tonghuashun";
var AKSHARE_BASE_URL = define_import_meta_env_default.VITE_AKSHARE_BASE_URL || "http://localhost:8000";
...
function prefixToMarket(prefix) {
  switch (prefix) {
    case "0":
    case "3":
      return "sz";
...
function parseTencentQuoteLine(symbol, raw) {
...
async function enrichQuoteFromTencent(secid, quote) {
  try {
    const symbol = secidToSymbol(secid);
    const res = await http3.get(`/tc/q=${symbol}`, { responseType: "text" });
...
async function main() {
  ...
  const enriched = await enrichQuoteFromTencent("1.601728", base);
  console.log("bids length:", enriched.bids?.length);
  console.log("asks length:", enriched.asks?.length);
  console.log("turnoverRate:", enriched.turnoverRate);
  console.log("amount:", enriched.amount);
  console.log("bid1:", enriched.bids?.[0]);
  console.log("ask1:", enriched.asks?.[0]);
  if (!enriched.bids?.length) throw new Error("bids empty");
  if (!enriched.asks?.length) throw new Error("asks empty");
  if (!Number.isFinite(enriched.turnoverRate)) throw new Error("turnoverRate invalid");
  console.log("orderbook enrichment test PASSED");
}
main().catch((e) => {
  console.error(e);
  process.exit(1);
